...
static const char *errorInvalidOutExtension =
    "output filename shall have .glTF, .glb, .stl or .obj extension.";
/// @}
...
public:
  virtual void Show(const Message_ProgressScope & /*theScope*/,
                    const Standard_Boolean /*force*/) override {
...
static int step2stl(char *in, char *out) {
  enum OutputFormat { Format_GLTF, Format_GLB, Format_STL, Format_OBJ };
...
static void show_usage(const char *app) {
  std::cerr << "stl2gltf - A tool to convert STEP to glTF with OpenCascade"
            << std::endl;
  std::cerr << std::endl;
  ...
  std::cerr << "    " << kLinearDeflection
            << "=FLOAT      Linear deflection (default: "
            << DefaultLinDeflection << ")" << std::endl;
  std::cerr << "    " << kAngularDeflection
            << "=FLOAT     Angular deflection (default: "
            << DefaultAngDeflection << ")" << std::endl;
  ...
  std::cerr << "    \".gltf\" - glTF file and bin file with resourses"
            << std::endl;
